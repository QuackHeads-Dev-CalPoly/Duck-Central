...
void send_lora_packet(BMP bmp388, bmx160SensorData magnetometer,
                      bmx160SensorData gyroscope,
                      bmx160SensorData accelerometer) {
    uint8_t buffer[255] = {};

    /*
        [sequence, temp (C), pressure (Pa), altitude (m), mag (uT), gyro (g),
       accel (m/s/s), lat, long]
        */
