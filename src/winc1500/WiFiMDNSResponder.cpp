...
bool WiFiMDNSResponder::begin(char* _name, uint32_t _ttlSeconds)
{
  ...
  // Replaced with my own version of to lowercase conversion
  int i;
...
bool WiFiMDNSResponder::parseRequest()
{
  ...
  if (packetLength) {
    ...
    if (memcmp_P(request, expectedRequestHeader, 4) == 0 &&                      // request header start match
        ...
        requestNameLength == strlen(name) &&                                    // name length match
        strncasecmp(name, (char*)requestName, requestNameLength) == 0 && // name match
