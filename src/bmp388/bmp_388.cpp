...
BMP::BMP() {
    ...
    int ret;
    uint8_t rx_data;
    ret = i2c_read_blocking(i2c0, addr, &rx_data, 1, false);
    if( ret < 0 )
        printf("Did not find BMP388 on bus\n");
    else
        printf("Found BMP388 on bus\n");
...
BMP3_INTF_RET_TYPE bmpWrite(uint8_t reg_addr, const uint8_t *read_data, uint32_t len, void *intf_ptr) {
    ...
    uint8_t con_buf[2] = {reg_addr, read_data[0]};
    int res;
    res = i2c_write_blocking(i2c0, addr, con_buf, 2, false);
    printf("write result %d\n", res);

    // Write to reg
    //int8_t write_res = i2c_write_blocking(i2c0, addr, read_data, len, false);
    return res > 0 ? BMP3_INTF_RET_SUCCESS : BMP3_E_COMM_FAIL;
}
